...
// 1. Add Redis import and Sub/Pub
content = content.replace(/import \{ JwtService \} from '@nestjs\/jwt';/, `import { JwtService } from '@nestjs/jwt';\nimport { Redis } from 'ioredis';`);

content = content.replace(/  private readonly isSwitching = new Set<string>\(\);/,
...
// 2. Vocab Forcing
content = content.replace(/const systemPrompt = await this\.buildAdaptivePrompt\(userId, profile, 'FREE_TALK'\);/g,
...
content = content.replace(/let systemPrompt = await this\.buildAdaptivePrompt\([\s\S]*?userId,\n        profile \|\| \{ currentLevel: 'A1', confidenceScore: '0\.5', vnSupportEnabled: 'true', tier: 'FREE' \},\n        mode,\n      \);/,
`const targetWords = await this.redis.getClient().lrange(\`vocab_force:\${userId}\`, 0, 2);
...
content = content.replace(/const systemPrompt = await this\.buildAdaptivePrompt\([\s\S]*?userId,\n        profile \|\| \{ currentLevel: 'A1', confidenceScore: '0\.5', vnSupportEnabled: 'true', tier: 'FREE' \},\n        data\.mode,\n        scenarioId,\n        topicId,\n        ragContext,\n      \);/,
`const targetWords = await this.redis.getClient().lrange(\`vocab_force:\${userId}\`, 0, 2);
...
content = content.replace(/  private async buildAdaptivePrompt\(\n    userId: string,\n    profile: Record<string, string>,\n    mode: string,\n    scenarioId\?: string,\n    topicId\?: string,\n    ragContext\?: string,\n  \): Promise<string> \{/,
...
content = content.replace(/\$\{groundedContext\}\n\n<security_shield>/,
...
// 3. UserTranscript Interceptions (Affective & IRT)
content = content.replace(/      client\.emit\('ai_transcript', \{ text: redactedUserText, isUser: true \}\); \/\/ Echo back to client/g,
...
// 4. Affective Computing timers
content = content.replace(/    emitter\.on\('turnComplete', \(\) => \{\n      client\.emit\('turn_complete'\);\n    \}\);/,
...
content = content.replace(/    this\.socketAudioEventCounts\.delete\(client\.id\);/g,
`    this.socketAudioEventCounts.delete(client.id);
    const silenceTimer = this.socketSilenceTimers.get(client.id);
    if (silenceTimer) clearTimeout(silenceTimer);
    this.socketSilenceTimers.delete(client.id);`);

// 5. Save placement score and add validMode
content = content.replace(/    \/\/ 4\. Batch update user token balance/g,
