...
function applyPatch() {
  let file = fs.readFileSync('apps/gateway-server/src/voice/voice.gateway.ts', 'utf8');

  // 1. Imports
  file = file.replace(/import \{ JwtService \} from '@nestjs\/jwt';/,
    `import { JwtService } from '@nestjs/jwt';\nimport { Redis } from 'ioredis';`);

  // 2. Class members
  file = file.replace(/  private readonly isSwitching = new Set<string>\(\);/,
...
`  private async initPlacementLevelSubscriber() {
  ...
  // 5. Vocab Forcing - Handle Connection
  file = file.replace(/      const systemPrompt = await this\.buildAdaptivePrompt\(userId, profile, 'FREE_TALK'\);\n      const sessionId = `\$\{userId\}_/g,
`      const targetWords = await this.redis.getClient().lrange(\`vocab_force:\${userId}\`, 0, 2);
      ...
      const sessionId = \`\${userId}_\``);

  // Vocab Forcing - Switch Mode
  file = file.replace(/      let systemPrompt = await this\.buildAdaptivePrompt\(\n        userId,\n        profile \|\| \{ currentLevel: 'A1', confidenceScore: '0\.5', vnSupportEnabled: 'true', tier: 'FREE' \},\n        mode,\n      \);/g,
`      const targetWords = await this.redis.getClient().lrange(\`vocab_force:\${userId}\`, 0, 2);
  ...
  // Vocab Forcing - Set Scenario
  file = file.replace(/      const systemPrompt = await this\.buildAdaptivePrompt\(\n        userId,\n        profile \|\| \{ currentLevel: 'A1', confidenceScore: '0\.5', vnSupportEnabled: 'true', tier: 'FREE' \},\n        data\.mode,\n        scenarioId,\n        topicId,\n        ragContext,\n      \);/g,
`      const targetWords = await this.redis.getClient().lrange(\`vocab_force:\${userId}\`, 0, 2);
  ...
  // Vocab Forcing - Signature
  file = file.replace(/    topicId\?: string,\n    ragContext\?: string,\n  \): Promise<string> \{/g,
`    topicId?: string,
    ragContext?: string,
    targetWords?: string[],
  ): Promise<string> {`);

  // Vocab Forcing - Template
  file = file.replace(/\$\{groundedContext\}\n\n<security_shield>/g,
...
<security_shield>`);
  ...
  // 6. UserTranscript Events
  file = file.replace(/      client\.emit\('ai_transcript', \{ text: redactedUserText, isUser: true \}\); \/\/ Echo back to client/g,
`      client.emit('ai_transcript', { text: redactedUserText, isUser: true }); // Echo back to client
  ...
  // 7. Affective Computing - Turn Complete Timer
  file = file.replace(/    emitter\.on\('turnComplete', \(\) => \{\n      client\.emit\('turn_complete'\);\n    \}\);/g,
`    emitter.on('turnComplete', () => {
  ...
  file = file.replace(/    this\.socketAudioEventCounts\.delete\(client\.id\);/g,
`    this.socketAudioEventCounts.delete(client.id);
    const silenceTimer = this.socketSilenceTimers.get(client.id);
    if (silenceTimer) clearTimeout(silenceTimer);
    this.socketSilenceTimers.delete(client.id);`);

  // 8. Save Placement Level on DB
  file = file.replace(/    \/\/ 4\. Batch update user token balance/g,
...
`      TOPIC_DISCUSSION:
  ...
  fs.writeFileSync('apps/gateway-server/src/voice/voice.gateway.ts', file);
}
